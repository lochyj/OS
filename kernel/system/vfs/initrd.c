#include "kernel/system/vfs/initrd.h"
...
struct fs_node* initrd_install(uint32_t location) {
	// Initalize the main and file header poitners and populate the root directory
	...
	for (i = 0; i < initrd_header->nfiles; i++) {
		...
		strcpy(root_nodes[i].name, (const uint8_t*)&file_headers[i].name, strlen((const uint8_t*)&file_headers[i].name));
